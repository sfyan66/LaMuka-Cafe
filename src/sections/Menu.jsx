...
function Menu() {
  ...
  return (
    <Element name="Menu" className="my-[180px]">
      <h1 className="text-[10vw] md:text-[min(5vw,72px)] text-white text-center pb-[60px]">
        Our Menu
      </h1>
      <ul className="flex gap-[15px] sm:gap-[30px] justify-center">
        {[
          ...
          <button
            className={active === btn.name ? "btnMActive" : "btnM"}
            onClick={() => {
              setActive(btn.name);
              setProducts(btn.value);
      ...
      <AnimatePresence mode="wait">
        <div className="md:max-w-7xl max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-y-[40px] sm:gap-y-16 gap-x-12 my-[40px]">
          {products.map((prodc) => {
            return (
              <motion.div
                key={prodc.title}
                className="flex sm:gap-6 gap-4 mx-auto"
                smooth={true}
              ...
              >
                <motion.img
                  src={prodc.img}
                  alt=""
                  className="h-[min(24vw,160px)] md:h-[min(14vw,160px)] block transition-all rounded-md"
                  smooth={true}
                />
                <div className="max-w-[360px] flex flex-col sm:gap-[25px] gap-[15px] justify-center">
                  <div className="flex justify-between">
                    <h4 className="text-white text-[min(4.2vw,24px)] md:text-[min(2.2vw,26px)]">
                      {prodc.title}
                    </h4>
                    <h5 className="text-[var(--button-second-bg)] text-[min(4.4vw,26px)] md:text-[min(2.5vw,28px)] underline font-bold">
                      {prodc.price}
                    </h5>
                  </div>
                  <p className="text-[var(--text-main-c)] text-[min(3.2vw,18px)] md:text-[min(1.5vw,18px)]">
                    {prodc.desc}
                  </p>
                </div>
              </motion.div>
