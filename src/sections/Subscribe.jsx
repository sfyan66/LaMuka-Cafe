    ...
    <div className='flex justify-center items-center pb-[100px] after:content[" "] after:bg-[var(--bg-card)] after:w-full after:h-[2px] after:absolute md:max-w-7xl max-w-5xl mx-auto after:md:max-w-7xl after:max-w-5xl'>
      <div className='relative bg-slate-100/5 shadow-[0_0_5px_rgba(255,255,255,0.20)] backdrop-blur-xl rounded-xl max-w-[460px] flex-1 md:max-w-[min(75vw,650px)] flex flex-col items-center py-[48px] px-[64px] gap-[48px] z-10'>
        ...
        <form className='flex gap-[16px] w-full relative flex-col md:flex-row' onSubmit={handleSubmit}>
          ...
          className='flex-1 bg-slate-700/20 placeholder:text-[var(--text-main-c)] sm:placeholder:text-[18px] placeholder:text-[16px] rounded-full sm:py-[20px] py-[12px] px-[24px] sm:px-[36px] text-white text-[18px] outline-none shadow-[0_0_3px_rgba(255,255,255,0.15)]'/>
          <button 
          type='submit' 
          disabled={isDisabled}
          className='bg-[var(--button-second-bg)] rounded-full sm:py-[20px] py-[12px] md:px-[48px] md:absolute top-0 right-0 text-white sm:text-[18px] text-[16px] cursor-pointer hover:bg-[var(--button-hover-bg)]'>Subscribe</button>
        </form>
        <div className='text-[#bf4042] mt-[-35px] text-[12px] sm:text-[16px]'>{inValidMessage}</div>
        <div className='text-[var(--button-second-bg)] text-[14px] sm:text-[20px] mt-[-35px] flex-1 md:flex-0'>{validMessage}</div>
      </div>
    </div>
  )
}
